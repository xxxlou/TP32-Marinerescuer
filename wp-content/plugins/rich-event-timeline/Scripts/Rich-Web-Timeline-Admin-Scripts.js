...
function rw_ti_edit(ti_id) 
{
	...
	jQuery.post(ajaxurl, data, function(response) {
		...
		jQuery('.RW_Timeline_ShowType_Val').val(arr[2]);
		jQuery('#rich_web_timeline_pp').val(arr[3]);
		jQuery('#rich_web_timeline_pp_span').html(arr[3]);
		jQuery('#RW_Timeline_LoadMore').val(arr[4]);
		jQuery('#RW_Timeline_Prev_Text').val(arr[5]);
		jQuery('#RW_Timeline_Next_Text').val(arr[6]);
		var allSHTypes = document.querySelectorAll(".RW_Timeline_ShowType")
		for( var i=0; i < allSHTypes.length; i++ ){
			if(arr[2] == allSHTypes[i].value){
...
function RW_TimeLine_Theme_Changed()
{
	...
	if(value == '2')
	{
		jQuery(".RW_Op_Title_Round").hide(); jQuery(".rw_timeline_year_border_col").hide(); jQuery(".rw_ti_month_opt").hide(); jQuery(".rw_timeline_icons_tr_st").hide(); jQuery(".rw_timeline_md_op_st").hide(); jQuery(".rw_timeline_line_col").show(); jQuery('.rw_timeline_data_format').hide();
	}
	if(value == '3')
	{
