...
namespace WPDataAccess\Utilities {
	...
	class WPDA_Table_Actions {
		...
		public function tab_settings() {
			...
			$row_count_is_configurable = strtolower( $engine ) === 'innodb' || strtolower( $engine ) === 'federated' || $engine === null;
			?>
			<style>
			...
			<table class="widefat striped rows wpda-structure-table">
				<tr>
					<td>
						<script type='text/javascript'>
							jQuery("#wpda_invisible_container").append("<?php echo $settings_table_top_form; // phpcs:ignore WordPress.Security.EscapeOutput ?>");
							jQuery("#wpda_invisible_container").append("<?php echo $settings_table_form; // phpcs:ignore WordPress.Security.EscapeOutput ?>");
							jQuery("#wpda_invisible_container").append("<?php echo $delete_menu_table_form; // phpcs:ignore WordPress.Security.EscapeOutput ?>");
						</script>
								...
								<li>
									...
									<ul class="wpda_table_settings_nested">
										<div style="font-size:90%;font-weight:bold;">
											<?php echo __( 'Row count (configurable for InnoDB tables, federated tables and views only)', 'wp-data-access' ); ?>
										</div>
										<div style="font-size:90%;">
											...
											<div style="padding: 5px 0 5px 25px">
												<label class="wpda_action_font">
													<input type="radio"
														   name="<?php echo esc_attr( $this->table_name ); ?>_row_count_estimated_value"
														   value="plugin"
															<?php
												...
												<label class="wpda_action_font">
													...
													if ( isset( $settings_db_custom->table_settings->row_count_estimate_value_hard ) ) {
														...
														$wpdadb  = WPDADB::get_db_connection( $this->schema_name );
														$explain = $wpdadb->get_results(
															'explain select count(*) from `' . str_replace( '`', '', $this->table_name ) . '`',
															'ARRAY_A'
														);
													...
													Use hard estimate:
													<input type="number"
														   id="<?php echo esc_attr( $this->table_name ); ?>_row_count_estimated_value_hard"
														   value="<?php echo esc_attr( $row_count ); ?>"
														   echo ! $row_count_is_configurable ? 'disabled ' : '';
												   />
													<a href="javascript:void(0)"
													   onclick="get_table_row_count('<?php echo esc_attr( wp_create_nonce( "wpda-get-row-count-{$this->table_name}" ) ); ?>', '<?php echo esc_attr( $this->schema_name ); ?>', '<?php echo esc_attr( $this->table_name ); ?>')"
											...
											<label class="wpda_action_font">
												...
												[<a href="<?php echo admin_url( 'options-general.php' ); // phpcs:ignore WordPress.Security.EscapeOutput ?>?page=wpdataaccess&tab=backend">change plugin default</a>]
											</label>
											<div style="padding-left: 25px">
												Performs real row count if estimate <= <?php echo esc_attr( WPDA::get_option( WPDA::OPTION_BE_INNODB_COUNT ) ); ?>.
												Uses estimated row count if estimate > <?php echo esc_attr( WPDA::get_option( WPDA::OPTION_BE_INNODB_COUNT ) ); ?>.
											</div>
										</div>
										<br/>
										<div style="font-size:90%;font-weight:bold;">
										...
										<table class="wpda_table_settings">
											<tr>
												...
												<?php foreach ( $column_settings_add_column as $add_column ) { ?>
													<th>
														<?php echo esc_attr( $add_column['label'] ); ?>
														<span class="dashicons dashicons-editor-help wpda_tooltip"
															  title="<?php echo esc_attr( $add_column['hint'] ); ?>"
															  style="cursor:pointer;vertical-align:text-bottom;"></span>
													</th>
												<?php } ?>
											</tr>
											...
											foreach ( $this->table_structure as $column ) {
												...
												<tr>
													...
													<?php foreach ( $column_settings_add_column as $add_column ) { ?>
														<td>
															<input
																type="<?php echo esc_attr( $add_column['type'] ); ?>"
																id="<?php echo esc_attr( $add_column['name_prefix'] ) . esc_attr( $column['Field'] ); ?>"
...
Variable $$column_name$$ will be replaced with the value of column $$column_name$$ in table `' . esc_attr( $this->table_name ) . '`.', 'wp-data-access' ); ?>"
											...
											if ( isset( $settings_db_custom->hyperlinks ) && is_array( $settings_db_custom->hyperlinks ) ) {
												foreach ( $settings_db_custom->hyperlinks as $hyperlink ) {
													...
													?>
													<tr id="wpda_<?php echo esc_attr( $this->rownum ); ?>_add_hyperlink_<?php echo esc_attr( $no_hyperlinks ); ?>">
														<td style="padding-left:0;vertical-align:top;">
															<input id="<?php echo esc_attr( $this->rownum ); ?>_hyperlink_label_<?php echo esc_attr( $no_hyperlinks ); ?>"
																   class="wpda_action_font"
																   type="text"
																   value="<?php echo esc_attr( $hyperlink_label ); ?>"
															/>
															<input id="<?php echo esc_attr( $this->rownum ); ?>_hyperlink_label_<?php echo esc_attr( $no_hyperlinks ); ?>_old"
																   type="hidden"
																   value="<?php echo esc_attr( $hyperlink_label ); ?>"
															/>
														</td>
														<td style="vertical-align:top;text-align:center;">
															<input id="<?php echo esc_attr( $this->rownum ); ?>_hyperlink_list_<?php echo esc_attr( $no_hyperlinks ); ?>"
																   class="wpda_action_font"
																   type="checkbox"
																   style="margin-top:7px;"
																   <?php echo esc_attr( $hyperlink_list ); ?>
															/>
														</td>
														<td style="vertical-align:top;text-align:center;">
															<input id="<?php echo esc_attr( $this->rownum ); ?>_hyperlink_form_<?php echo esc_attr( $no_hyperlinks ); ?>"
																   class="wpda_action_font"
																   type="checkbox"
																   style="margin-top:7px;"
																   <?php echo esc_attr( $hyperlink_form ); ?>
															/>
														</td>
														<td style="vertical-align:top;text-align:center;">
															<input id="<?php echo esc_attr( $this->rownum ); ?>_hyperlink_target_<?php echo esc_attr( $no_hyperlinks ); ?>"
																   class="wpda_action_font"
																   type="checkbox"
																   style="margin-top:7px;"
																   <?php echo esc_attr( $hyperlink_target ); ?>
															/>
														</td>
														<td style="vertical-align:top;">
															<textarea id="<?php echo esc_attr( $this->rownum ); ?>_hyperlink_html_<?php echo esc_attr( $no_hyperlinks ); ?>"
																	  rows="5"
																	  class="wpda_action_font"
																	  style="min-width:300px;resize:both;"
															><?php echo urldecode( $hyperlink_html ); // phpcs:ignore WordPress.Security.EscapeOutput ?></textarea>
															<input id="<?php echo esc_attr( $this->rownum ); ?>_hyperlink_html_<?php echo esc_attr( $no_hyperlinks ); ?>_old"
																   type="hidden"
																   value="<?php echo $hyperlink_html; // phpcs:ignore WordPress.Security.EscapeOutput ?>"
															/>
														</td>
														<td style="vertical-align:top;">
															<a href="javascript:void(0)"
															   ...
															   onclick="if (confirm('<?php echo sprintf( __( 'Delete hyperlink %s?', 'wp-data-access' ), esc_attr( $hyperlink_label ) ); ?>')) { jQuery('#wpda_<?php echo esc_attr( $this->rownum ); ?>_add_hyperlink_<?php echo esc_attr( $no_hyperlinks ); ?>').remove(); }"
															></a>
														</td>
													</tr>
													<?php
										...
										<div>
											...
											<input id="no_hyperlink_<?php echo esc_attr( $this->rownum ); ?>"
												   type="hidden" value="<?php echo esc_attr( $no_hyperlinks ); ?>">
										</div>
									</ul>
								</li>
								<li>
									...
									<ul class="wpda_table_settings_nested">
										<table class="wpda_table_settings">
											...
											foreach ( $table_menus as $table_menu ) {
												...
												<tr>
													<td style="padding-left:0;vertical-align:top;">
														<input id="menu_name_<?php echo esc_attr( $no_menus ); ?>"
															   class="wpda_action_font" type="text"
															   value="<?php echo esc_attr( $table_menu['menu_name'] ); ?>"
															   style="font-size: 90%;"
														>
														<input id="menu_name_<?php echo esc_attr( $no_menus ); ?>_old" type="hidden"
															   value="<?php echo esc_attr( $table_menu['menu_name'] ); ?>">
													</td>
													<td style="vertical-align:top;">
														<input id="menu_slug_<?php echo esc_attr( $no_menus ); ?>"
															   class="wpda_action_font" type="text"
															   value="<?php echo esc_attr( $table_menu['menu_slug'] ); ?>"
															   style="font-size: 90%;"
														>
														<input id="menu_slug_<?php echo esc_attr( $no_menus ); ?>_old" type="hidden"
															   value="<?php echo esc_attr( $table_menu['menu_slug'] ); ?>">
													</td>
													<td>
														<select id="menu_roles_<?php echo esc_attr( $no_menus ); ?>"
														...
														>
															...
															foreach ( $wp_roles->roles as $role => $val ) {
																...
																echo "<option value='$role' $selected>$role_label</option>"; // phpcs:ignore WordPress.Security.EscapeOutput
															}
															?>
														</select>
														<a href="javascript:void(0)"
														   class="dashicons dashicons-trash"
														   onclick="if (confirm('<?php echo sprintf( __( 'Delete menu %s?', 'wp-data-access' ), esc_attr( $table_menu['menu_name'] ) ); ?>')) { jQuery('#<?php echo esc_attr( $this->rownum ); ?>_menu_id').val('<?php echo esc_attr( $table_menu['menu_id'] ); ?>'); jQuery('#<?php echo esc_attr( $delete_menu_table_form_id ); ?>').submit(); }"
														></a>
														<input id="menu_id_<?php echo esc_attr( $no_menus ); ?>"
															   class="wpda_table_setting_item_menu" type="hidden"
															   value="<?php echo esc_attr( $table_menu['menu_id'] ); ?>">
														<input id="menu_role_<?php echo esc_attr( $no_menus ); ?>_old" type="hidden"
											...
											>
												...
												<td>
													...
													>
														...
														foreach ( $wp_roles->roles as $role => $val ) {
															$role_label = isset( $val['name'] ) ? $val['name'] : $role;
															echo "<option value='$role'>$role_label</option>"; // phpcs:ignore WordPress.Security.EscapeOutput
														}
														?>
													</select>
													<input id="menu_id_<?php echo esc_attr( $no_menus ); ?>"
							...
							<input class="wpda_table_setting_item" type="hidden" id="wpda_<?php echo esc_attr( $this->rownum ); ?>_sql_dml"
								   value="<?php echo esc_attr( $sql_dml ); ?>"
							/>
						</div>
					</td>
				</tr>
			...
			<script type='text/javascript'>
				jQuery(function () {
					...
					jQuery('#wpda_<?php echo esc_attr( $this->rownum ); ?>_save_table_settings').click( function() {
						return submit_table_settings(
							...
							'<?php echo esc_attr( $settings_table_top_form_settings ); ?>',
							'<?php echo esc_attr( $settings_table_top_form_id ); ?>',
					...
					jQuery('#wpda_<?php echo esc_attr( $this->rownum ); ?>_save_column_settings').click( function() {
						return submit_column_settings(
							'<?php echo esc_attr( $this->rownum ); ?>',
							'<?php echo esc_attr( $settings_table_form_settings ); ?>',
							'<?php echo esc_attr( $settings_table_form_id ); ?>'
						);
					});
				...
				<?php
				foreach ( $column_settings_add_column as $add_column ) {
					echo 'custom_column_settings.push("' . $add_column['name_prefix'] . '");'; // phpcs:ignore WordPress.Security.EscapeOutput
				}
				?>
			</script>
			<?php
		...
		protected function tab_sql() {
			...
			<table class="widefat striped rows wpda-structure-table">
				<tr>
					<td>
						<?php echo wp_kses( $this->create_table_stmt, array( 'br' => array() ) ); ?>
					</td>
					<td style="text-align: right;">
						<a id="button-copy-clipboard-<?php echo esc_attr( $this->rownum ); ?>"
						   href="javascript:void(0)"
						   class="button button-primary"
						   data-clipboard-text="<?php echo $this->create_table_stmt_orig; // phpcs:ignore WordPress.Security.EscapeOutput ?>"
						>
			...
			<tr>
				<td style="box-sizing:border-box;text-align:center;white-space:nowrap;width:150px;vertical-align:middle;">
					<a href="javascript:void(0)"
					   target="_blank"
					   class="button button-primary"
					   onclick="return wpda_export_button_<?php echo esc_attr( $this->rownum ); ?>()"
					   style="display:block;"
					>
				...
				<td style="vertical-align:middle;">
					...
					<select id="format_type_<?php echo esc_attr( $this->rownum ); ?>"
						...
						if ( stripos( $this->table_name, $wpdb->prefix ) === 0 ) {
							...
							?>
							<option value="sqlpre" <?php echo $export_variable_prefix_option ? 'selected' : ''; ?>>SQL (add WP prefix)</option>
							<?php
						}
						?>
					...
					<script type="text/javascript">
						function wpda_export_button_<?php echo esc_attr( $this->rownum ); ?>() {
							<?php
							...
							?>
							if (<?php echo $check_export_access; // phpcs:ignore WordPress.Security.EscapeOutput ?>) {
								wpda_table_export(
									'<?php echo esc_attr( $this->schema_name ); ?>',
									'<?php echo esc_attr( $this->table_name ); ?>',
									'<?php echo esc_attr( $wp_nonce ); ?>',
		...
		protected function tab_copy() {
			...
			<tr>
				<td style="box-sizing:border-box;text-align:center;white-space:nowrap;width:150px;vertical-align:middle;">
					<script type='text/javascript'>
						jQuery("#wpda_invisible_container").append("<?php echo $copy_table_form; // phpcs:ignore WordPress.Security.EscapeOutput ?>");
					</script>
					<a href="javascript:void(0)"
					   class="button button-primary"
					   onclick="if (jQuery('#copy-table-from-<?php echo esc_attr( $this->rownum ); ?>').val()==='') { alert('<?php echo __( 'Please enter a valid table name', 'wp-data-access' ); ?>'); return false; } if (confirm('<?php echo __( 'Copy', 'wp-data-access' ) . ' ' . esc_attr( strtolower( $this->dbo_type ) ) . '?'; ?>')) { jQuery('#copy_table_name_<?php echo esc_attr( $this->table_name ); ?>').val(jQuery('#copy-table-from-<?php echo esc_attr( $this->rownum ); ?>').val()); jQuery('#<?php echo esc_attr( $copy_table_form_id ); ?>').submit(); }"
					   style="display:block;"
					>
		...
		protected function tab_truncate() {
			...
			<tr>
				<td style="box-sizing:border-box;text-align:center;white-space:nowrap;width:150px;vertical-align:middle;">
					<script type='text/javascript'>
						jQuery("#wpda_invisible_container").append("<?php echo $truncate_table_form; // phpcs:ignore WordPress.Security.EscapeOutput ?>");
					</script>
					<a href="javascript:void(0)"
					   class="button button-primary"
					   onclick="if (confirm('<?php echo __( 'Truncate table?', 'wp-data-access' ); ?>')) { jQuery('#<?php echo esc_attr( $truncate_table_form_id ); ?>').submit(); }"
					   style="display:block;"
					>
		...
		protected function tab_drop() {
			...
			<tr>
				<td style="box-sizing:border-box;text-align:center;white-space:nowrap;width:150px;vertical-align:middle;">
					<script type='text/javascript'>
						jQuery("#wpda_invisible_container").append("<?php echo $drop_table_form; // phpcs:ignore WordPress.Security.EscapeOutput ?>");
					</script>
					<a href="javascript:void(0)"
					   class="button button-primary"
					   onclick="if (confirm('<?php echo $msg_drop; // phpcs:ignore WordPress.Security.EscapeOutput ?>')) { jQuery('#<?php echo esc_attr( $drop_table_form_id ); ?>').submit(); }"
					   style="display:block;"
					>
		...
		protected function tab_optimize() {
			...
			<tr>
				<td style="box-sizing:border-box;text-align:center;white-space:nowrap;width:150px;vertical-align:middle;">
					<script type='text/javascript'>
						jQuery("#wpda_invisible_container").append("<?php echo $optimize_table_form; // phpcs:ignore WordPress.Security.EscapeOutput ?>");
					</script>
					<a href="javascript:void(0)"
					   class="button button-primary"
					   onclick="if (confirm('<?php echo $msg_optimize; // phpcs:ignore WordPress.Security.EscapeOutput ?>')) { jQuery('#<?php echo esc_attr( $optimize_table_form_id ); ?>').submit(); }"
					   style="display:block;
					   <?php
		...
		protected function tab_alter() {
			...
			<tr>
				<td style="box-sizing:border-box;text-align:center;white-space:nowrap;width:150px;vertical-align:middle;">
					<script type='text/javascript'>
						jQuery("#wpda_invisible_container").append("<?php echo $alter_table_form; // phpcs:ignore WordPress.Security.EscapeOutput ?>");
					</script>
					<a href="javascript:void(0)"
					   class="button button-primary"
					   onclick="if (confirm('<?php echo __( 'Alter table?', 'wp-data-access' ); ?>')) { jQuery('#<?php echo esc_attr( $alter_table_form_id ); ?>').submit(); }"
					   style="display:block;"
					>
